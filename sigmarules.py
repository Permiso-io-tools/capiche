...
def generate_sigma_rule(rule_name, description, api_data, api_list, sdk, useragent):
    transformed_api_list = transform_api_list(api_data, sdk, api_list)['APIListTransformed']
...
detection:
  selection:
    eventSource: 
      - {'\n      - '.join([f"'{source}'" for source in sigma_rule['detection']['selection']['eventSource']])}
    eventName: 
      - {'\n      - '.join([f"'{name}'" for name in sigma_rule['detection']['selection']['eventName']])}
...
if __name__ == "__main__":
    ...
    parser.add_argument("-RuleName", required=True, help="Rule name")
    parser.add_argument("-Description", required=True, help="Description for the Sigma rule")
    parser.add_argument("-APIList", nargs='+', required=True, help="List of APIs in the format 'EventSource:EventName'")
    parser.add_argument("-SDK", required=True, choices=['boto', 'awscli'], help="SDK type (boto or awscli)")
    parser.add_argument("-UserAgent", required=True, help="User agent to match the selected SDK")
